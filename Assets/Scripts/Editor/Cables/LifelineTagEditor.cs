...
[CustomEditor(typeof(LifelineTag))]
public class LifelineTagEditor : Editor {

    protected virtual void OnSceneGUI()
    {
        return;

        LifelineTag marker = (LifelineTag)target;
        ...
        if (Handles.Button(oldPosition + (markerTransform.forward * 0.5f), Quaternion.LookRotation(direct), 0.1f, 0.2f, Handles.ConeHandleCap))
        {
            ...
            Vector3 rotCorrect = markerTransform.rotation.eulerAngles;
            rotCorrect.x = 0;
            rotCorrect.z = 0;
            markerTransform.rotation = Quaternion.Euler(rotCorrect);
        }
