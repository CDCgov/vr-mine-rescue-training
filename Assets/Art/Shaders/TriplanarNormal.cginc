    ...
    out float4 Out)
{
    ...
    float3 Node_X = UnpackNormal(SAMPLE_TEXTURE2D(TextureA, Sampler, Node_UV.zy));
    float3 Node_Y = UnpackNormal(SAMPLE_TEXTURE2D(TextureB, Sampler, Node_UV.xz));
    float3 Node_Z = UnpackNormal(SAMPLE_TEXTURE2D(TextureC, Sampler, Node_UV.xy));
